...
public class QuebraCabeca {
	...
	public static void main(String[] args) {
		System.out.println("*****ORDENE O QUEBRA CABEÇA*****");
	...
	public static void realizarMovimento(int matriz[][]) {
		System.out.print("Informe o movimento (2 (^), 4 (<), 6 (>) ou 8 (v)):");
			...
			for (int i = 0; i < matriz.length && !sair; i++) {
				for (int j = 0; j < matriz[0].length; j++) {
					if (matriz[i][j] == 0 && tecla == 8 && (i - 1) >= 0) {
						...
						sair = true;
						break;
					} else if (matriz[i][j] == 0 && tecla == 2 && (i + 1) <= 2) {
						numeroDeMovimentos++;
						matriz[i][j] = matriz[i + 1][j];
						matriz[i + 1][j] = 0;
						i++;
